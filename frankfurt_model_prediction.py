...
from frankfurt_data_preparation import read_file, show_statistics, inspect_dataset
...
if __name__ == "__main__":
    ...
    # subset_size = 100
    image_dataset, mask_dataset = read_file(data_dir, image_dir, mask_dir, multiclass=False)
    ...
    result_dir = os.path.expanduser("~/Documents/Omdena/omdena-frankfurt-ugs-unet/experiment08_decrease_weight_decay")
    model = tf.keras.models.load_model(os.path.join(result_dir, 'unet.keras'), compile=False)  # Load trained model
    ...
    result_dir_eval = 'frankfurt_predictions'
    os.makedirs(result_dir_eval, exist_ok=True)
    run_evaluation(model, X_test, y_test, result_dir_eval)
