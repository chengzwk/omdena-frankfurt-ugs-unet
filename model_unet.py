...
def unet_model(input_shape, use_dropout=True):
    inputs = Input(input_shape)

    s1, p1 = encoder_block(inputs, 64, use_dropout=use_dropout)  # number of filters can be customized
    s2, p2 = encoder_block(p1, 128, use_dropout=use_dropout)
    s3, p3 = encoder_block(p2, 256, use_dropout=use_dropout)
    s4, p4 = encoder_block(p3, 512, use_dropout=use_dropout)

    b1 = conv_block(p4, 1024, use_dropout=use_dropout) #Bridge

    d1 = decoder_block(b1, s4, 512, use_dropout=use_dropout)
    d2 = decoder_block(d1, s3, 256, use_dropout=use_dropout)
    d3 = decoder_block(d2, s2, 128, use_dropout=use_dropout)
    d4 = decoder_block(d3, s1, 64, use_dropout=use_dropout)
